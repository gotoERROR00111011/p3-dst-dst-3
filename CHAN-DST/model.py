...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.attn = MultiHeadAttention(
            self.attn_head, self.bert_output_dim, dropout=0.1
        )
        self.attn2 = MultiHeadAttention(
            self.attn_head, self.bert_output_dim, dropout=0.1
        )
        self.add_pe = PositionalEncoding(self.bert_output_dim, 0.0)
        ...
        self.nbt = Encoder(
            EncoderLayer(
                ...
                PositionwiseFeedForward(
                    self.bert_output_dim, self.hidden_dim, self.hidden_dropout_prob
                ),
    ...
    def initialize_slot_value_lookup(self, label_ids, slot_ids):
        ...
        hid_slot, _ = self.sv_encoder(
            input_ids=slot_ids.view(-1, self.max_label_length),
            token_type_ids=slot_type_ids.view(-1, self.max_label_length),
        ...
        for s, label_id in enumerate(label_ids):
            ...
            hid_label, _ = self.sv_encoder(
                input_ids=label_id.view(-1, self.max_label_length),
                token_type_ids=label_type_ids.view(-1, self.max_label_length),
    ...
    def forward(
        ...
        hidden = torch.mul(
            hidden, attention_mask.view(-1, seq_length, 1).expand(hidden.size()).float()
        )
        ...
        for d in range(ds):
            ...
            turn_mask[d] = padding_utter.unsqueeze(0).repeat(ts, 1) & subsequent_mask(
                ts
            ).to(self.device)
        ...
        if update is not None:
            ...
            hidden_update = torch.tanh(
                self.clsf_update(
                    F.dropout(hidden, p=self.args.mt_drop, training=self.training)
                )
            )
            # relu activation
                ...
                self.update_linear(
                    torch.cat(
                        [
                            ...
                            torch.cat(
                                [
                                    torch.zeros_like(hidden_update[:, :1]).cuda(),
                                    hidden_update[:, :-1],
                                ],
                                1,
                            ),
            ...
            prob_update = (
                prob_update.view(slot_dim, ds, ts).permute(1, 2, 0).contiguous()
            )
            mask = (update > -1).float()
            update_1 = update.masked_fill((1 - mask).bool(), 1)
            prob_update_1 = prob_update.masked_fill((1 - mask).bool(), 1)
            loss_update = F.binary_cross_entropy(
                prob_update_1, update_1, reduction="none"
            )
        ...
        if n_gpu == 1:
            return (
                ...
                acc_slot,
                pred_slot,
                (main_loss, loss_update, acc_update),
            )
        else:
            return (
                loss.unsqueeze(0),
                ...
                pred_slot,
                main_loss.unsqueeze(0),
                loss_update.unsqueeze(0),
                acc_update.unsqueeze(0),
            )
