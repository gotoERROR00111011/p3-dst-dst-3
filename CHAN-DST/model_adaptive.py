...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        self.attn = MultiHeadAttention(
            self.attn_head, self.bert_output_dim, dropout=0.0
        )
    ...
    def forward(
        self, input_ids, input_len, labels, n_gpu=1, target_slot=None, slot_alpha=None
    ):
        ...
        hidden, _ = self.utterance_encoder(
            input_ids.view(-1, max_seq_length),
        ...
        hidden = torch.mul(
            hidden,
            attention_mask.view(-1, max_seq_length, 1).expand(hidden.size()).float(),
        )
        ...
        hidden = self.attn(
            ...
            hidden,
            hidden,
            mask=attention_mask.view(-1, 1, max_seq_length).repeat(slot_dim, 1, 1),
        )
                ...
                if self.training is True:
                    if slot_alpha is not None:
                        _loss = self.nll(
                            _dist.view(ds * ts, -1),
                            labels[:, :, s].view(-1),
                            alpha=slot_alpha[s] if slot_alpha is not None else 1,
                        )
                    else:
                        _loss = self.nll(
